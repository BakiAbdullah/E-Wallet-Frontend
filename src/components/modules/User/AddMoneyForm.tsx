...
"use client";
import TopUpImg from "@/assets/images/TopUp.jpg";
...
export function AddMoneyForm({
  ...
  const [topUpMoney] = useTopUpWalletMutation();
  ...
  const onSubmit = async (data: TopUpFormValues) => {
    ...
    try {
      const result = await topUpMoney(userInfo).unwrap();
      console.log({ result });
